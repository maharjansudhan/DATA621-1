...
[[[COMMENT - PLEASE INTRODUCE, GIVE SOME LOGICAL EXPLANATION FOR HOW THESE MODELS ARE ARRIVED AT]]]
...
lm1 <- lm(TARGET_WINS ~  I(TEAM_BATTING_H + TEAM_BATTING_BB + TEAM_BATTING_HBP) + 
              I(TEAM_BASERUN_SB - TEAM_BASERUN_CS), df_train)
(lm1sum <- summary(lm1))
```
...
lm2 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_BB + TEAM_BASERUN_SB + 
              TEAM_FIELDING_E + TEAM_PITCHING_H + TEAM_PITCHING_BB + 
              TEAM_PITCHING_SO, df_train)
(lm2sum <- summary(lm2))
```
...
lm3b <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_2B + TEAM_BATTING_HR + 
               TEAM_BATTING_BB + TEAM_BATTING_SO + TEAM_BATTING_HBP, df_train)
...
lm6 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_HR + TEAM_BATTING_BB + 
              ...
              TEAM_PITCHING_H + TEAM_PITCHING_HR + TEAM_PITCHING_BB, df_train)
(lm6sum <- summary(lm6))
```
...
lm7 <- lm(TARGET_WINS ~ I(TEAM_BATTING_H + TEAM_BATTING_BB) + TEAM_BATTING_HR + 
              ...
              I(TEAM_PITCHING_H + TEAM_PITCHING_BB) + TEAM_PITCHING_HR, df_train)
(lm7sum <- summary(lm7))
```
...
models <- list(lm0, lm1, lm2, lm3, lm3a, lm3b, lm4, lm5, lm6, lm7)
modsums <- list(lm0sum, lm1sum, lm2sum, lm3sum, lm3asum, lm3bsum, lm4sum, 
                lm5sum, lm6sum, lm7sum)
nmod <- length(modsums)
...
modnames <- paste0("lm", c(0:3, "3a", "3b", 4:7))
...
for (j in 1:length(var_list))
    plot(rstandard(lm6) ~ var_list[[j]], ylab = "Standardized Residual",
         xlab = var_names[j])
```
