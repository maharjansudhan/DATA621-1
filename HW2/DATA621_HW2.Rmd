...
```{r}
#to cross check the 0s and 1s and see the difference

print(df$scored.class)

print(df$class)


raw_confusion_matrix <- table(df$class,df$scored.class)
raw_confusion_matrix

```
Here, we have Actual and Predicted class, the rows can be called Acutal class and and columns can be called Predicted class even though they have different meanings and represents different values.
...
accuracy <- function(df){
  ...
  accuracy_value <- (confusion_matrix$Freq[1] + confusion_matrix$Freq[4])/sum(confusion_matrix$Freq)
